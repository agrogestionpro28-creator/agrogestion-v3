...
export default function CuadernoCampo() {
  ...
  const guardarCosecha = async () => {
    setGuardando(true)
    if ((cosecha as any).id) {
      await supabase.from('lote_cosecha').update(cosecha).eq('id', (cosecha as any).id)
    } else {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              {[
                ...
                <div key={key}>
                  ...
                  {editandoLote ? (
                    <input value={loteEdit[key] || ''} onChange={e => setLoteEdit({ ...loteEdit, [key]: e.target.value })} style={inputStyle} />
                  ) : (
              ...
              <div>
                ...
                {editandoLote ? (
                  <input type="date" value={loteEdit.fecha_siembra_real || ''}
                    onChange={e => setLoteEdit({ ...loteEdit, fecha_siembra_real: e.target.value })} style={inputStyle} />
                ) : (
                  <div style={{ color: '#f5f0e8', fontSize: '14px', padding: '8px 0' }}>
              ...
              <div>
                ...
                {editandoLote ? (
                  <input type="date" value={loteEdit.fecha_cosecha_real || ''}
                    onChange={e => setLoteEdit({ ...loteEdit, fecha_cosecha_real: e.target.value })} style={inputStyle} />
                ) : (
                  <div style={{ color: '#f5f0e8', fontSize: '14px', padding: '8px 0' }}>
        ...
        {tab === 'fertilizacion' && (
          <div>
            <p style={{ color: '#a09070', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>Fertilizaciones</p>
        ...
        {tab === 'cosecha' && (
          <div>
            <p style={{ color: '#a09070', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>Datos de cosecha</p>
